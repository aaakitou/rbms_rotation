...
float rbms::pid(float T,short rpm_now, short set_speed,float *delta_rpm_pre,float *ie_spd,float KP, float KI,float KD )//pid制御
{
    ...
    *ie_spd = *ie_spd + (delta_rpm + *delta_rpm_pre)*T/2;
    float out_torque  = KP*delta_rpm + KI*(*ie_spd) + KD*de;
...
float rbms::pid_deg(float T,short deg_now, short set_deg,float *delta_deg_pre,float *ie_deg,float KP, float KI,float KD )//pid制御
{
    ...
    *ie_deg = *ie_deg + (delta_deg + *delta_deg_pre)*T/2;
    float out_speed  = KP*delta_deg + KI*(*ie_deg) + KD*de;
        ...
        for(int id=0;id<_motor_num;id++){
            if(_msg.id==0x201+id){//esc idごとに受信データ割り振り
                ...
                if(_motor_type){
                    ...
                    motor[id] = (int)pid(tm[id].read(),speed[id]/36,set_speed[id],&delta_rpm_pre[id],&ie_spd[id],15,6);
                }
                tm[id].reset();//timer reset
                if(motor[id]>_motor_max){motor[id]=_motor_max;}else if(motor[id]<-_motor_max){motor[id]=-_motor_max;}//上限確認超えてた場合は上限値にset
